...
import {preloadFonts} from "../../../BlockSignPage/AboutBlockSign/codeAni/utils";
...
export async function initScrollAni(){
  ...
  console.log('fx1Titles', fx1Titles)
  ...
  const scroll = () => {
    ...
    fx4Titles.forEach(title => {
      ...
      for (const word of words) {
        ...
        gsap.fromTo(chars,  {
            'will-change': 'opacity, transform',
            x: (position:any,_:any,arr:any) => 150*(position-arr.length/2)
          },
          {
            ease: 'power1.inOut',
            x: 0,
    ...
    fx9Titles.forEach(title => {
      ...
      for (const word of words) {
        ...
        gsap.fromTo(chars,  {
            'will-change': 'transform',
            scaleX: 0,
            x: (_:any, target:any) => window.innerWidth/2 - target.offsetLeft - target.offsetWidth/2
          },
          {
            ease: 'power1.inOut',
            scaleX: 1,
