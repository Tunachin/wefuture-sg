...
export default async function scrollAni2(){
    ...
    fx21Titles.forEach(title => {
      ...
      for (const word of words) {
        ...
        gsap.fromTo(chars, {
            'will-change': 'opacity, transform',
            opacity: 0,
            y: (position: number, _: any, arr: string | any[]) => -40*Math.abs(position-arr.length/2),
    ...
    fx23Titles.forEach(title => {
      ...
      for (const [wordPosition, word] of words.entries()) {
        ...
        gsap.fromTo(word.querySelectorAll('.char'), {
            'will-change': 'transform',
            scale: 0.01,
            x: (pos: number, _: any, arr: string | any[]) => {
